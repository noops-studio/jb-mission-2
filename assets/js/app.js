...
(_a = document.getElementById('searchForm')) === null || _a === void 0 ? void 0 : _a.addEventListener('submit', async (e) => {
    e.preventDefault();
...
(_b = document.getElementById('allButton')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', fetchAllCountries);
...
function displayStatistics(countries) {
    ...
    const regionTableRows = Object.entries(regionStats).map(([region, count]) => {
        return `<tr>
                    <td>${region}</td>
                    <td>${count}</td>
                </tr>`;
    ...
    const currencyTableRows = Object.entries(currencyStats).map(([currency, count]) => {
        return `<tr>
                    <td>${currency}</td>
                    <td>${count}</td>
                </tr>`;
